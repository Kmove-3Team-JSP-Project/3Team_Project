...
public class ItemDao {
	public Item insert(Connection conn, Item item) throws SQLException {
		...
		try {
			pstmt = conn
					.prepareStatement("insert into ITEM_ID, ITEM_NAME, UNIT_PRICE, ITEM_CLASS) values (?, ?, ?, ?)");
	...
	public ItemPage select(Connection conn, int startRow, int size) throws SQLException {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = conn.prepareStatement("select * from (select rownum as rnum, a.* "
					+ "from (select * from item order by item_name desc) a " + "where rownum <= ?) where rnum >= ?");
	...
	public Item convertItem(ResultSet rs) throws SQLException {
		return new Item(rs.getInt("item_no"), rs.getString("item_name"), rs.getInt("unit_price"),
